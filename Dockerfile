...
# ClaudioASantana/hermes-agent fork customization: turn on the supervised
# dashboard s6 service (docker/s6-rc.d/dashboard/run) by default on :8000,
# replacing the old `CMD ["dashboard", ..., "--insecure"]` override from the
# pre-s6 image. NOTE: HERMES_DASHBOARD_INSECURE no longer disables the auth
# gate on a non-loopback bind (upstream hardening after the June 2026
# MCP-config persistence campaign) — the service starts but reports down
# until an auth provider is supplied at `docker run` time via
# HERMES_DASHBOARD_BASIC_AUTH_USERNAME + _PASSWORD, or
# HERMES_DASHBOARD_OAUTH_CLIENT_ID. See website/docs/user-guide/docker.md.
ENV HERMES_DASHBOARD=1
ENV HERMES_DASHBOARD_PORT=8000
